...
Extracto Reflexion 6.6 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.6) se califica con rubrica: Cherry-pick (max 4), Gestion de conflictos (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece cherry_pick_checks.cp2_cherry_pick_simple.ok en true, reconoce explicitamente el cherry-pick simple.
- Si en JSON aparece cherry_pick_checks.cp3_cherry_pick_multiple.ok en true, reconoce explicitamente el cherry-pick multiple.
- Si en JSON aparece cherry_pick_checks.cp4_resolucion_accion.ok en true, reconoce explicitamente el uso de `--continue` o `--abort`.
- Si en JSON aparece cherry_pick_checks.cp4_conflicto.ok en true, reconoce explicitamente la gestion del conflicto.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-6.md'))
